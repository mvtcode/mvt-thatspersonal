...
namespace uniform.Impl
{
    public class imageAlbumImpl : IimageAlbum
    {
        ...
        public List<imageAlbumInfo> GetAllByType(int AlbumId, out int total)
        {
            List<imageAlbumInfo> list = null;
            var t = 0;
            SqlParameter[] param = {
            ...
            SqlCommand comx;
            var r = DataHelper.ExecuteReader(Config.ConnectString, "usp_imageAlbum_GetAllByType", param, out comx);
            if (r != null)
            {
                list = new List<imageAlbumInfo>();
                while (r.Read())
